...
Prepare the tables of modiifed and unmodified water entities - cleaning the water entities names
```{r}
...
2) Join the areas table with both score table by water entity id.
...
At the same time, there are 10 entities, that are in the status table but are not listed in the shape files.
...
```
I will create another areas file where I will manually add these 10 entities, and join with scores table once again.
```{r}
areas_addition <- read_excel("data/entities_not_in_shape_file.xlsx")
areas_addition <- areas_addition %>%  select(-area_km2)
...
Of the total 1230 records of status, there are 69 in this table than did not have neither ecological nor chemical status assessed. 
I will gapfill this missing values to have more data and not to distort weighting of water entities within a municipality.


# Gapfilling 69 NA scores 

The list of "empty" records with no ecological or chemical score assessed:
```{r}
no_status_areas <- scores_per_mcp %>% 
  filter(is.na(score)) 
datatable(no_status_areas)
```
 
To fill the missing values, I will use mean imputation per river-coast system areas (also called drainage areas). Usually, they cotain 15-30 water entities.
According to Norwegian Water entities authority, vassdrag area is the area from which a system of rivers drains into a particular area of a coast. So, if we think about land-based pollution, the status of water entities within one vassdrag area can be expected to be similar, but  resolution maybe too large of there is very local pollution.
...
However, this approach will solve a problem of NA only where there are more than 1 observation per drainage area.  Example - drainage area 228, entity *storfjorden_0422020300_c* is a single assessed entity in the whole drainage area. Drainage area 227 - all three entities did not have status assessed. Thereare in total 5 such occasions. For this 5 occasinons, I will just replace NA with a score of closest water entity. 
...
shapes_north <- entity_shapes[entity_shapes$Vannregion %in% c("Finnmark", "Troms", "Nordland"), ] 
```
...
Merge sorted water entities with the scores.Becuase there are duplicate of water entities in score_per_mcp table (due to one entity extends over several municipalities), I group by entity and draiange area and take only one of the duplicates. I use drainage area as a grouping variable, because there are occasions (e.g. Akkarfjord) when entities has same IDs but a located in different drainage areas.
```{r}
...
Gapfill the 5 missing values using last-observation-carried-forward approach:
```{r}
...
Next, I group the data by municipality and caclulate the total area of all water entities within a municipality and porpotional area of each water entity per municipality.
```{r}
