...
water_mcp <- status_prep3 %>% 
  select(water_entity_id, kom1:kom9) %>% 
  pivot_longer(.,
               cols = kom1:kom9,
...
water_mcp_prep3 <- water_mcp_prep2 %>% 
  ...
  select(-Kid) %>% 
  select(municip_name, municip_number, municip_number_old, water_entity_id)
```
...
For strongly modified areas, bad is 0, moderate is 25, good is 50.
...
**Status for strongly modified areas**
```{r}
...
First, for a strongly modified areas:
```{r}
modified_num_status <- modified %>%  
  select(water_entity_id, okologisk_potensial) %>% 
  mutate(num_status = pmap_dbl(
