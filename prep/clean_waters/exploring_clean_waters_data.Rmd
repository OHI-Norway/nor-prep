...
There is a mess with municipalities' numbers. Some of them are written as 3 digits (which is wrong!), because a tailing zero is missing and I have to manually add it. All municipalities' numbers starting with "50**" can be deleted - they belong to Trondelag county.
```{r}
...
water_mcp_prep3 <- water_mcp_prep2 %>% 
  ...
  select(-Kid) %>% 
  select(municip_name, municip_number, municip_number_old)
```
```{r}
