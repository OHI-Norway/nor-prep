...
Have a look at the distribution of aquaculture goal, when q75 and q90 used as reference yileds
```{r}
...
```
We see clearly that using absolute maximal yield is it not meaningful - only a few (12) municipalities could reach the highest score.
...
```
Save the table with aquaculture goal:
```{r}
