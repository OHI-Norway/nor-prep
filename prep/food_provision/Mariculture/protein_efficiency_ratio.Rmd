...
# Calculating protein efficiency 
```{r}
...
protein_prep3 <- protein_prep2 %>%
  ...
  mutate_at(c(5,6,7,8,9,10), as.numeric) %>%     #mutate all the columns that have to be numeric but are character!
  rowwise() %>%
  mutate(total_prod =  sum(trout_prod_kg, salmo_prod_kg, na.rm = T)) %>%
  mutate(protein_eff = estimate_per(
...
# Save final table 
```{r}
#write_csv(protein_prep3, "data/protein_efficiency_ratio.csv")
```
