...
```
Regarding localities with no municipality info: they are either shut down or were not yet given licence to start the produciton (commented by Fisheries Directorate). 
Thus, I delete these locaitons. 
...
#write.csv(examinations_filtered, "data/number_momb_examinations_per_mpp.csv")
...
#Evene in 2010-2018, about 220 localities did not have information on the capacity (not sure why, bad quality of data I think)
...
```{r include=FALSE}
#Old approach - scaling momb from 1-4 to 0 to 1.
# momb_prep4 <- momb_prep3 %>%
...
```{r}
#Gapfilling with the last 5 years of data if there are at least 7 years of data, otherwise - gapfilling with all aivailable years.
...
```
Almost half of the municipalities-year observations (411 out of 910) have momb index of 1. This is because momb grade of 3 or 4 is very rare in northern Norway, so in most cases, every municipality has localities with momb grade 1 or 2. 
