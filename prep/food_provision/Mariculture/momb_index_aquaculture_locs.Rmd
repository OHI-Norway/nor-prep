...
# Estimating running mean of the mean momb per municipality
Remove municipalities with no data
```{r}
momb_bymp <- split(momb_prep4, momb_prep4$Kommune) 

#rollapply(df.mp$total_prod_corrected, width = 3, FUN = mean,  fill=NA))

#check if there are municipalities with empty rows.
#There are 13 of such municipalities
empties<- map(momb_bymp, ~ isTRUE(length(.x$momb_mean)==0))
...
```{r}
long_data <- map(momb_bymp_cleaned, ~ length(.x$momb_mean)) %>% 
  unlist(.) %>% 
  stack(.) %>% 
  filter(values >= 5) %>% 
  select(-values) %>% 
...
short_data <- map(momb_bymp_cleaned, ~ length(.x$momb_mean)) %>% 
  unlist(.) %>% 
  stack(.) %>% 
  filter(values < 5) %>% 
  select(-values) %>% 
...
long_momb_smoothed <- long_momb_data %>% 
  map(.,~ as.data.frame(.x)) %>% 
  map(., ~ smooth_df(.x, 4)) %>% 
  map(., ~ mutate(.x, momb_mean=smoothed)) %>% 
