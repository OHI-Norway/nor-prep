...
The scores will be based on 90th percentile of the annual aquaculture yield (although we could also use the maximal yield). 
...
scores_final_prep2 <-scores_final_prep %>% 
  ...
  select(c(year:municip, sustainability, aquaculture_score, aquacutlure_score_q90, Norw_name))
```
...
Plotting aquaculture score per municipality and county
...
plot <- ggplot(df)+
      geom_line(aes(x = year, y = aquacutlure_score_q90), size = 0.7, color = "darkblue") +
...
ggsave(paste0("figs/aquaculture_final_score_q90_", cty, ".pdf"), plot)
