...
#find row numbers Salangen 2011 - remove this value
...
plot <- ggplot(data = sustprod3, aes(x = Yrs, y = Tons, fill = Type)) +
      geom_col()+
      facet_wrap(Kname  ~ .) +
      scale_x_continuous(breaks = breaks1, labels = labels1)+ 
      labs(y = "Sustainable mariculture prod (tons)", x = "")+
...
ggsave(paste0("figs/sustainable_production_proportion",".png"), plot, width = 6, height = 6) 
