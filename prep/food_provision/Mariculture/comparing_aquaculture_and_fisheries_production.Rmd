...
```{r}
fishcatch <- vroom::vroom(file.path("../fisheries/data/catch_total.csv"))
aquaprod <-vroom::vroom(file.path("data/gapfilled_aquaculture_production.csv"))
```

# Clean the datasets and merge tby municipality and year
```{r}
fishcatch_prep <- fishcatch %>% 
  ...
  mutate(municip_number = replace(municip_number, municip %in% c("Bjarkoy_2013", "Harstad_2013"), 1903)) %>% 
  mutate(municip_number = replace(municip_number, municip == "Skjerstad_2005", 1804)) %>% 
...
counties_df <- split(totprod_prep, totprod_prep$county)
...
plots_food <- map2(
  ...
  ~ ggplot(
    ...
    labs(y = "Total food production (tons)", x = "") +
    theme_food() +
