...
colnames(escap_prep) <-c("county", "company", "location_number","species", "size_kg", "date", "year", "count_escaped", "count_esc_fishdir", 
                         "farmed", "recatched", "escaped_final")

escap_prep <- filter(escap_prep, !county %in% c("M","H","NT","ST","SF","R","AA"))

lusedata <-read.csv(file.path(dir_M[2], "Aquaculture/Lice/lakselus_per_fisk_utf8.csv"), fileEncoding = "UTF-8")
...
#Some observations for salmon and cod are from differnet locations within the same municipality and year, but no more than 4. 
...
escap_spp <-escap_fin  %>% 
             ...
             summarize(count_escaped = sum(count_escaped),count_esc_fishdir = sum(count_esc_fishdir), 
                       produced_municip = sum(produced), recathced = sum(recatched), escaped_municip = sum(escaped_final))
