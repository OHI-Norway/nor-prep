...
# Cleaning aquculture production data
```{r data}
...
akva_final <- akva_prep2 %>%
  select(-c(16:33)) %>%
  filter_at(vars(c(8:15)), all_vars(!is.na(.))) %>% 
  clean_names(.)   
 

#write.csv(akva_final, "data/total_mariculture_production.csv")
```

# Deleting negative produciton numbpers 
```{r}
...
# Plot total log(aquaculture production)
