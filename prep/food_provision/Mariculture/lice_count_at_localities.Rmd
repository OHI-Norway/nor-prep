...
There are 80 locations for which no municipality and region specified, also the lice threshold is missing for those.
...
foo <-lus_prep %>% group_by(Ar) %>% 
  mutate(lusegrense = as.numeric(as.character(Lusegrense.uke)))


foo %>%  
  group_by(Ar) %>% 
  summarize(grensemin = min(lusegrense),
            grensemax = max(lusegrense))
```


Merging lus_prep with lisences dataframe - to find municipaities where they are missing. 
I could find info only for 2 out of 89 locations with missing municipality.
...
foo2 <-filter(lus_prep, Over.lusegrense.uke == "Ukjent")
foo2 <- foo2 %>% 
...
#write.csv(total_locations,"data/number_lice_obersvations_per_mpp.csv") 
```
...
```
About halth of the locations reported salmon lice only once a year. Barentswatch.no explains that missing lice count maybe because the locality did not have fish in the pens that weeks or they just did not report. I will select only the locations that reported at least 25 weeks over a year (about half of all locations).
...
# Estimate average % of weeks when localities are below lice threshold (old data)
```{r}
...
# Gapfilling missing data in lice dataseet
...
#13 counties 
```
All these municipalities have 0 produciton in the Akvadatut Per's table but I found that indeed most of these municipalities had aquaculture licenses in the 2005-2018 period. Is there a mistake? Did they produce something or not? Need to check wiht Per.
...
lice_gapfilled <- do.call("rbind", lice_replaced_prep) %>%
  rename(Municip_number = Komnum_new, Municip = Kommune_new, momb_gapfilled = replaced) %>%
  mutate(gf = ifelse(is.na(below_lice), "y", "n"))

#write_csv(lice_gapfilled, "data/lice_below_thr_gapfilled.csv")
```

Final table compiled, now need to check with the 13 mpp with no data and comfirm that my methods of gapfillig is ok.