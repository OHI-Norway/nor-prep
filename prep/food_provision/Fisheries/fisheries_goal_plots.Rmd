...
scores_final <- read.table()
scores_final_prep <-scores_final %>% 
  left_join(komnames_norw[c("Komnum", "Norw_name")], by = ("Municip_number" = "Norw_name"))
```

```{r}

sc_counties <- unique(scores_final_prep$county) 

for(i in 1:length(sc_counties))
  {
  cty <- sc_counties[i]
  
  df <- scores_final_prep %>%
    filter(county == cty) 
    
    
plot <- ggplot(df)+
      geom_line(aes(x = Year, y = final_score), size = 0.7, color = "darkblue") +
...
ggsave(paste0("figs/fisheries_sustainability_score_", cty, ".pdf"), plot)
