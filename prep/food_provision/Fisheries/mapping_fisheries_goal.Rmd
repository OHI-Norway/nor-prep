...
```{r }
fishscore <-read.csv("./data/fishery_final_score.csv")#read the relevant file, this is jsut an example
fishscore <-fishscore %>% rename("fish_score"= "fihs_score")
```
...
```{r }
fishscore94 <-filter(fishscore, Year == "1994")
foo <- left_join(kommunersf, fishscore94, by = c("KOMM" ="Municip_number"))
plot_fishscore <- st_as_sf(foo)
```
...
coloredplot3 <- ggplot(data = world) +
  geom_sf()+
   ...
   geom_sf(data = plot_fishscore, aes(fill = fish_score), color = "black", size = 0.5) + 
  scale_fill_viridis_c(option = "C", alpha = 0.85, begin=0.1, end = 0.7, name= " ")+
...
##  Defniing the general map theme (some items will be customized later)
```{r}
theme_map <- function(...) {
  theme_minimal() +
  theme(
...
#  Trying 3D tricks to plot the score
```{r}
library(rayshader)
```

```{r}
plot_gg(coloredplot3, raytrace = TRUE, width = 10, height = 8, scale = 100, windowsize = c(1400, 866), 
         multicore = TRUE, fov = 50, zoom = 0.9, phi = 45, theta = 20,  sunangle = 300)
render_camera(fov = 50, zoom = 0.5, theta = 50, phi = 45)
```
