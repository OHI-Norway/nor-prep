...
krabnew_prep <- krabnew %>%
  ...
  mutate(English = replace(Art_gruppe, Art_gruppe %in% c("Kongekrabbe, han", "Kongekrabbe, annen"), "Kongekrabbe")) %>%
  ...
  mutate(Total_catch = gsub(",", MeasureValues, replacement = ".")) %>%
  ...
  mutate(Total_catch = as.numeric(Total_catch)) %>% 
  select(-c(
  ...
  select(Year,Municip_number,Municip_name, English, Total_catch)
...
```{r}
catch_merged <- bind_rows(catch_final, krabnew_prep)
#write_csv(catch_merged,"data/fish_catch_by_municipality_newcrab.csv")
```
